...
## CORE METHODS ################################################################
    ...
    def plotBoundary(self, X, Y):
        ...
        des_x = [np.amin(X[:, 0]), np.amax(X[:, 0])]
        des_y = [
            (des_x[0]*self.theta.item(1) + self.theta.item(0))/-self.theta.item(2),
            (des_x[1]*self.theta.item(1) + self.theta.item(0))/-self.theta.item(2)
                 ]
    ...
    def predict(self, X):
        ...
        if dpoints is 0:
            return []
        assert X.shape[1] is 2, 'Prediction requires input to have exactly 2 features'

        to_return = np.zeros(dpoints)
        for i in range(0, dpoints):
            if self.theta[0] + self.theta[1]*X[i, 0] + self.theta[2]*X[i, 1] > 0:
                to_return[i] = self.classes[1]
            else:
                to_return[i] = self.classes[0]
        return to_return
